(function($) {
	$.fn.extend({
		...
		if (options.weekStart == '星期天') {
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>星期天</div><div>星期一</div><div>星期二</div><div>星期三</div><div>星期四</div><div>星期五</div><div>星期六</div></div><div class="monthly-day-wrap"></div>');
		} else if (options.weekStart == '星期一') {
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>星期一</div><div>星期二</div><div>星期三</div><div>星期四</div><div>星期五</div><div>星期六</div><div>星期天</div></div><div class="monthly-day-wrap"></div>');
		} else {
		...
		function setMonthly(m, y){
			...
			if (setMonth == currentMonth && setYear == currentYear) {
				...
				$('#' + uniqueId + ' .monthly-header-title').html(monthNames[m - 1] +' '+ y +'<a href="#" class="monthly-reset" title="返回当前月"></a> ');
			}

			// Account for empty days at start
			if(options.weekStart == '星期天' && firstDay != 7) {
				for(var i = 0; i < firstDay; i++) {
					$('#' + uniqueId + ' .monthly-day-wrap').prepend('<div class="m-d monthly-day-blank"><div class="monthly-day-number"></div></div>');
				}
			} else if (options.weekStart == '星期一' && firstDay == 0) {
				for(var i = 0; i < 6; i++) {
					$('#' + uniqueId + ' .monthly-day-wrap').prepend('<div class="m-d monthly-day-blank" ><div class="monthly-day-number"></div></div>');
				}
			} else if (options.weekStart == '星期一' && firstDay != 1) {
			...
			if (options.mode == 'event') {
				...
				// Add Events
				options.xmlCount=2;
				//for(var a=1;a<=options.xmlCount;a++){
					//options.xmlUrl='/eventXML/event'+a+'.xml';
					$.get(''+options.xmlUrl+'', function(d){
										$(d).find('event').each(function(){
											...
											/* Convert times to 12 hour & determine 上午 or 下午 */
											...
											function multidaylist(){
												$('#'+uniqueId+' .monthly-list-item[data-number="'+i+'"]').addClass('item-has-event').append('<a href="'+eventURL+'" class="listed-event"  data-eventid="'+ eventId +'" style="background:'+eventColor+'" title="'+eventTitle+'">'+eventTitle+'<div><div class="monthly-list-time-start">'+startPeriod+' '+startTime+'</div><div class="monthly-list-time-end">'+endPeriod+' '+endTime+'</div></div></a>');
											}
		...
		function viewToggleButton(){
			if($('#'+uniqueId+' .monthly-event-list').is(":visible")) {
				...
				$('#'+uniqueId+' .monthly-header-title').prepend('<a href="#" class="monthly-cal" title="回到月视图"><div></div></a>');
			}
		}
